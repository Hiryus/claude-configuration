...
# The new `(cwd, previous_cwd)` a successful `cd` produces.
# Both are per-shell in bash, so both inherit and are discarded on exactly the same scope boundaries.
Move = tuple[Path, Path | None]

# TODO: what if the command fails?
# Ex: `rm tmp; cd tmp; cat ../etc/passwd`
#
# TODO: let's just deny the "cd -" command: it complexifies the code quite a bit.
# The agent can always do `cd <old_path>` instead.
...
def validate(command:CommandLine, context:Context) -> tuple[Decision, str, Move | None]:
    ...
    `cd` is a special command whose verdict also produces state, so this returns a 3-tuple (verdict, reason, move) and must never be fed to `worst()`.
    """
    ...
    # `cd -` goes back to $OLDPWD, which bash keeps per shell.
    ...
    return (Decision.ALLOW, f"changing directory to `{target}` is allowed.", (target, context.current_cwd))
