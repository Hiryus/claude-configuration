...
import claude.utils.session
from claude.user_prompt_submit import AUTO_MODE_NOTE, main
from claude.utils.session import read_mode, write_mode
...
def sessions(tmp_path, monkeypatch) -> Path:
    ...
    monkeypatch.setattr(claude.utils.session, "SESSIONS_DIR", directory)
    return directory
