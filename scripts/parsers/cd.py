...
GRAMMAR = CommandSyntax(
    aliases=["cd"],
    flags=[
        # The two flags that only pick how symlinks are read: neither changes where we land
        # in a way the hook can't follow. Anything else (-e, -@, ...) stays untabled so it
        # surfaces in `unknown` and the move is refused rather than mistracked.
...
def parse(command_line:CommandLine) -> Invocation:
    """
    Note on `cd -`: a bare `-` is classified as an *unknown flag*, not as an operand
    (`arguments.parse` splits on the leading dash). The analyzer detects it there.
