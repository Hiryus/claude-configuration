...
Create files lazily — only when you have something to write. If no `LANGUAGE.md` exists, create one when the first term is resolved. If no `docs/adr/` exists, create it when the first ADR (Architecture Decision Record) is needed.
...
Don't couple `LANGUAGE.md` to implementation details. Only include terms that are meaningful to domain experts.
